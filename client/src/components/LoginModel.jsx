...
function LoginModel({open,onclose}) {
 ...
 const handleGoogleAuth = async ()=>{
  ...
  return (
   ...
   justify-center bg-black/80 backdrop-blur-xl px-4'
   initial={{opacity:0}}
   animate={{opacity:1}}
   exit={{opacity:0}}
   onClick={()=>onclose()}
   ...
   transition={{duration:0.45,ease:"easeOut"}}>
    ...
    <div className='relative rounded-3xl bg-[#0b0b0b] 
    border border-white/10 shadow-[0_30px_120px_rgba(0,0,0,0.8)] overflow-hidden'>
      
      <motion.div
      animate={{opacity:[0.24,0.4,0.25]}}
      ...
      <button 
      onClick={onclose}
       className='absolute top-5 right-5 z-20 text-zinc-400 
      hover:text-white transition text-lg'>
        ❌
      </button>

      <div className='relative px-8 pt-14 pb-10 text-center'>
        <h1 className='inline-block mb-6 px-4 py-1.5 rounded-full 
        bg-white/5 border-white/10 text-xs text-zinc-300'>
        ...
        bg-clip-text text-transparent'>NexSite</span>
        </h2>
        ...
        whileTap={{scale:0.96}}
        className='group relative w-full h-13 rounded-xl
        bg-white text-black font-semibold shadow-xl overflow-hidden'
        >
